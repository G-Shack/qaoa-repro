...
Use Pytest whenever you tweak the core modules:
...
The suite asserts deterministic instance generation, validates statistical summaries, and checks knapsack evaluation logic.

## 8. Troubleshooting

- **Module import errors:** Ensure the notebook prep cell (adds `src/` to `sys.path`) has run; otherwise restart the kernel.
- **Qiskit/Aer issues:** The project relies on the versions pinned in `requirements.txt`. Re-install if simulations crash.
- **Matplotlib TypeErrors when saving tables:** Update to the latest repo version; the dashboard helper now handles mixed dtypes safely.
- **Long runtimes:** Reduce `shots`, `p` values, or grid density in the notebooks for quick experimentation.

## 9. Contributing / Extending

1. Branch from `main` and keep changes scoped.
2. Add or update tests in `tests/` when touching core logic.
3. Document new behavior either here or directly in the notebooks so future readers can reproduce the workflow.

Happy benchmarking! Open issues or pull requests if you uncover gaps, want to add new cost functions, or port the workflows to hardware backends.
